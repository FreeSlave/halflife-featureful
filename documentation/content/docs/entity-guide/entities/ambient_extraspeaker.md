...
This entity can be used for radio transmissions or other sounds that are supposed to play in the player's head, i.e. without the attenuation affecting the sound. Just like `ambient_generic` it can play sentences if a string with a leading `!` is set instead of the path to the wav file.
...
* `Channel to use` - the channel of the extra speaker to use. Playing a sound through another `ambient_extraspeaker` on the same entity and on the same channel will replace previous sound if any is played.
