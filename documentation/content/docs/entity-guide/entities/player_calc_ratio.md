...
* `Calc Param Type`
    ...
    - `Maximum speed` - current player's maximum speed. If fraction is requested the *current max speed* / *default max speed* is returned. The current maximum speed can be changed via [player_speed]({{< ref player_speed >}}).
