...
Let pitdrones jump freely using the [jumping]({{< ref "entity-templates/#jumping" >}}) property.

Note: the `jump` animation in the default **models/pit_drone.mdl** features some vertical motion by itself and also sets the vertical velocity via the animation event. So ideally it should be changed to a different animation; this is just an example.
