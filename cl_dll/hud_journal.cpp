...
void ParseQuestTexts(char* pfile, int length, const char* objectType, const char* fileName)
{
	...
	while ( i<length )
	{
		...
		else
		{
			...
			const char* existingText = gHUD.m_messageStrings.GetText(name);
			if (existingText)
			{
				gEngfuncs.Con_Printf("%s: message %d is already registered!\n", fileName, name);
			}
			else
			{
				//gEngfuncs.Con_Printf("%s: registering %s: %s!\n", fileName, name, message);
				gHUD.m_messageStrings.SetText(name, message);
			}
		}
	}
}
