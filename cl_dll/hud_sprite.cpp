...
float ScaledRenderer::GetScreenSafeArea() {
	assert(hud_renderer != NULL && hud_safe_area != NULL);

	float safe_area = 0.0f;

	if (hud_renderer->value > 0.0f && hud_safe_area->value > 0.0f) {
		safe_area = ScaleScreen(hud_safe_area->value);
	}

	return safe_area;
}

/*template<typename T> void ScaledRenderer::ClampToScreenWidthSafeArea(T *x, T *width) {
	if (float safe_area = GetScreenSafeArea()) {
		if (*x < safe_area) {
			*x = safe_area;
		}

		if (*x + *width > ScreenWidthScaled() - safe_area) {
			*x = ScreenWidthScaled() - safe_area - *width;
		}
	}
}*/

/*template<typename T> void ScaledRenderer::ClampToScreenHeightSafeArea(T *y, T *height) {
	if (float safe_area = GetScreenSafeArea()) {
		if (*y < safe_area) {
			*y = safe_area;
		}

		if (*y + *height > ScreenHeightScaled() - safe_area) {
			*y = ScreenHeightScaled() - safe_area - *height;
		}
	}
}*/

int ScaledRenderer::ScreenSafeAreaBounds(int value) {
	if (float safe_area = GetScreenSafeArea()) {
		if (safe_area > value) {
			return safe_area;
		}
