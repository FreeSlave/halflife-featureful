...
constexpr float RED_WEIGHT = 0.3f;
constexpr float GREEN_WEIGHT = 0.5f;
constexpr float BLUE_WEIGHT = 0.2f;

int ClosestPaletteColorIndex(const Color3 color)
{
	float minDistanceSquared = 255 * 255 * 3;
	int closestColor = 0;
	...
	for (size_t i = 0; i<256; ++i)
	{
		...
		if ((i >= 224 || closestColor/16 != 6) ? (distanceSquared < minDistanceSquared) : (distanceSquared <= minDistanceSquared))
		{
			closestColor = i;
