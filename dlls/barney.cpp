...
public:
	...
	int FollowerType() override { return FOLLOWER_TYPE_SECURITY_GUARD; }
...
{
	const int nogunSubdmoel = m_gunGroupModels >= 4 ? OTIS_BODY_GUNGONE : OTIS_BODY_GUNHOLSTERED;
	if ( g_pGameRules->FMonsterCanDropWeapons(this) && !FBitSet(pev->spawnflags, SF_MONSTER_DONT_DROP_GUN) && GetBodygroup(OTIS_GUN_GROUP) != nogunSubdmoel )
	{
		Vector vecGunPos;
		Vector vecGunAngles;

		SetBodygroup(OTIS_GUN_GROUP, nogunSubdmoel);
